...
def nlpanalysis(country):
    ...
    if(country == "Italy"):
        ...
        long_string = " ".join(w for w in nltk.wordpunct_tokenize(long_string) if w.lower() in words or not w.isalpha())

    long_string = re.sub(r'\b\w{1,3}\b', '', long_string)
    wordcloud = WordCloud(width=800, height=400, collocations = True, stopwords=stopwords, background_color="white",  normalize_plurals=True,  max_words=60, contour_width=5, contour_color='steelblue',colormap=matplotlib.cm.gist_heat)
    ...
    plt.savefig('/home/saad/Data/Twitter/country/'+ country + '/plots/'+ country +'_cloud2.pdf', dpi=400, bbox_inches='tight')
    # y  = wordcloud.words_
    # import operator
